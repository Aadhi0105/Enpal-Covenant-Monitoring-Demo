...
Facility B's own, correct Delinquency Ratio is the sum of the 91--120 and 120+ day balances (its 90-or-more-days definition), divided by the fixed Original Pool Balance. The ``if Facility A's definition were misapplied'' column below instead sums the 61--90, 91--120, and 120+ day balances (Facility A's 60-or-more-days definition) and divides by the current, amortising Pool Balance -- using the identical raw data, computed the wrong way.

\textbf{Computed results.} The same underlying raw delinquency data was computed two different ways: correctly, using Facility B's own contractual definition, and incorrectly, using Facility A's definition instead -- the exact mistake someone would make reusing a familiar definition without checking the governing terms of the specific facility in front of them.
